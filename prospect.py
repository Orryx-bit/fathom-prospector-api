...
Production-Hardened Version 3.1
- Robots.txt caching + graceful fallback enrichment
- Smarter deep scraping with prioritization and early stopping
- Data confidence flags and better logging hygiene
"""
...
class FathomProspector:
    ...
    def __init__(self, demo_mode=False, existing_customers_csv=None, progress_callback=None):
        ...
        self.model_name = "gpt-4o-mini"  # Default model: fast and cost-efficient

        if AI_AVAILABLE:
            logger.info('✓ Abacus RouteLLM ready - AI features enabled')
        ...
        # Robots cache: domain -> (can_scrape: bool, expires_at: datetime)
        self.robots_cache: Dict[str, Tuple[bool, datetime]] = {}
        self.robots_ttl_seconds = int(os.getenv("ROBOTS_TTL_SECONDS", "86400"))  # 24h
        ...
        try:
            ...
            for place in places_result.get('results', []):
                ...
                if self.is_hospital_system(place.get('name', '')):
                    logger.debug(f"Skipping hospital system: {place.get('name', '')}")
                    continue
                ...
                # Strict medical business filtering
    ...
    def is_medical_business(self, place_data: Dict) -> bool:
        """
        Strict filtering: Check if a place is a legitimate medical/aesthetic business using Google Places types
        """
        ...
        if not has_medical_type:
            logger.debug(f"Filtered (no medical type): {name} - Types: {place_types}")
            return False
        ...
        if any(ptype in place_types for ptype in exclude_types):
            logger.debug(f"Filtered (non-medical): {name} - Types: {place_types}")
            return False
        ...
        if any(keyword in name for keyword in exclude_keywords):
            logger.debug(f"Filtered (excluded keyword): {name}")
            return False

        logger.debug(f"Passed filter: {name} - Types: {place_types}")
        return True
    ...
    def _robots_cache_get(self, domain: str) -> Optional[bool]:
        entry = self.robots_cache.get(domain)
        if not entry:
            return None
        can_scrape, expires_at = entry
    ...
    def _robots_cache_set(self, domain: str, can_scrape: bool):
        self.robots_cache[domain] = (can_scrape, datetime.utcnow() + timedelta(seconds=self.robots_ttl_seconds))

    def can_scrape_url(self, url: str) -> bool:
        """Cached robots.txt check; default True on errors"""
        try:
            if not url:
                return False
            parsed = urlparse(url)
            ...
            cached = self._robots_cache_get(domain)
            ...
            self._robots_cache_set(domain, allowed)
            return allowed
        except Exception:
            # On any failure, be conservative + allow (we still handle gracefully downstream)
            return True
    ...
    def _enrich_without_scrape(self, place_data: Dict) -> Dict[str, any]:
        """
        Alternate enrichment path when scraping is blocked by robots.txt.
        Uses only safe data already available (e.g., Google Places details) and heuristics.
        No new web requests to the target domain are made.
        """
        name = place_data.get('name', 'Medical Practice')
        rating = place_data.get('rating', 0)
        review_count = place_data.get('user_ratings_total', 0)
        types = [t.lower() for t in place_data.get('types', [])]

        inferred_services = []
        # Heuristic mapping from Google types to likely services
        if 'spa' in types or 'beauty_salon' in types:
            inferred_services += ['botox', 'fillers', 'skin tightening']
        if 'doctor' in types or 'health' in types:
            inferred_services += ['laser hair removal', 'body contouring']
        inferred_services = list(dict.fromkeys(inferred_services))  # dedupe, preserve order

        description = f"{name} offers cosmetic and aesthetic services." if inferred_services else "Not Available - Restricted"

        return {
            'title': name,
            'description': description,
            'services': inferred_services,
            'social_links': [],  # unknown without scraping
            'staff_count': 0,
            'blocked_by_robots': True,
            'scrape_strategy': 'no_scrape_enrichment'
        }
    ...
    def scrape_website(self, url: str) -> Dict[str, any]:
        ...
        if self.demo_mode:
            ...
            data = self.get_mock_website_data(url)
            data.update({'blocked_by_robots': False, 'scrape_strategy': 'single_page'})
            return data
        ...
        if not self.can_scrape_url(url):
            ...
            return {
                **self._enrich_without_scrape({'name': 'Medical Practice', 'rating': 0, 'user_ratings_total': 0, 'types': []}),
                'scrape_strategy': 'no_scrape_enrichment'
            }
        ...
        try:
            ...
            data = {
                ...
                'staff_count': 0,
                'blocked_by_robots': False,
                'scrape_strategy': 'single_page'
            }

            # Get page title
            data['title'] = (soup.title.string.strip() if soup.title and soup.title.string else 'Not Available')
            ...
            if meta_desc and meta_desc.get('content'):
                ...
                data['description'] = (og_desc.get('content', '').strip() if og_desc and og_desc.get('content') else 'Not Available')
            ...
            data['services'] = [kw for kw in service_keywords if kw in text_content]
                ...
                for domain, platform_name in social_platforms.items():
                    if domain in href_lower and platform_name not in seen_platforms:
                        ...
                        if href.startswith('//'):
                            ...
                            # Relative URL - skip as not a social link
                            continue
                        social_links.append(full_url)
                        seen_platforms.add(platform_name)
                        logger.debug(f"Found {platform_name} link: {full_url}")
                        break
            data['social_links'] = social_links

            # Estimate staff count from staff indicators
            ...
            return {
                ...
                'staff_count': 0,
                'blocked_by_robots': False,
                'scrape_strategy': 'single_page_timeout'
            }
        except Exception as e:
            ...
            return {
                ...
                'staff_count': 0,
                'blocked_by_robots': False,
                'scrape_strategy': 'single_page_error'
            }
    ...
    def detect_specialty_ai(self, practice_data: Dict) -> str:
        ...
        try:
            ...
            website_text = practice_data.get('website_text', '')[:500]
...
Return only the one word."""
            ...
            valid = {'dermatology', 'plastic_surgery', 'obgyn', 'medspa', 'familypractice', 'general'}
            if result in valid:
    ...
    def analyze_pain_points_ai(self, practice_data: Dict, specialty: str) -> Dict:
        """AI-powered pain point analysis; falls back to rule-based"""
        if not self.ai_enabled:
...
READINESS_SCORE: [0-100]"""
    ...
    def generate_outreach_ai(self, practice_data: Dict, specialty: str, pain_analysis: Dict) -> Dict:
        """AI-powered outreach generation; falls back to template-based"""
        if not self.ai_enabled:
        ...
        try:
            ...
            pain_point_1 = pain_points[0] if len(pain_points) > 0 else ""
            pain_point_2 = pain_points[1] if len(pain_points) > 1 else ""
            pain_point_3 = pain_points[2] if len(pain_points) > 2 else ""

            prompt = f"""Generate exactly three outreach versions for this practice.
...
Pain Points: {pain_point_1} | {pain_point_2} | {pain_point_3}
Recommended Device: {device_rec}

COLD_CALL:
[2-3 minute opener script]
...
EMAIL_BODY:
[120-150 words, professional, concise, two time windows, low-friction alternative]
"""
            system_msg = "You are a top-performing medical device sales professional crafting personalized, effective multi-channel outreach."
            ...
            if 'COLD_CALL:' in response:
                ...
                response = 'INSTAGRAM:' + parts[1] if len(parts) > 1 else response

            if 'INSTAGRAM:' in response:
                parts = response.split('INSTAGRAM:', 1)[1].split('EMAIL_SUBJECT:', 1)
                instagram = parts[0].strip()
                response = 'EMAIL_SUBJECT:' + parts[1] if len(parts) > 1 else response
            ...
            return {
                'outreachColdCall': cold_call or None,
                'outreachInstagram': instagram or None,
                'outreachEmail': email_body or None,
                'outreachEmailSubject': email_subject or None,
...
Our Venus systems complement your existing practice by adding high-demand services like body contouring, cellulite reduction, and skin tightening—all with FDA-cleared technology.
...
Our systems offer body contouring, skin tightening, cellulite reduction, and wrinkle reduction—high-demand services that generate excellent revenue.
...
Venus Sales Team"""
        ...
        instagram = f"Hi! Noticed {name}'s reputation. Many practices like yours are seeing great results with Venus technologies. Would you be open to learning more?"
    ...
    def _score_link_priority(self, href: str) -> int:
        """Score internal links by likelihood they contain services/treatments/info"""
        if not href:
            return 0
        href_l = href.lower()
        score = 0
        keywords = [
            'service', 'treatment', 'procedure', 'offer', 'aesthetic', 'cosmetic',
            'medspa', 'spa', 'skin', 'laser', 'body', 'contour', 'weight', 'face',
            'menu', 'pricing', 'packages'
        ]
        for kw in keywords:
            if kw in href_l:
                score += 2
        # Prefer shorter slugs and non-parameterized URLs
        if '?' not in href_l and '#' not in href_l:
            score += 1
        return score

    def discover_site_pages(self, base_url: str, homepage_html: Optional[str] = None) -> List[str]:
        """
        Discover high-value pages on a practice website; prioritize likely treatment pages.
        """
        urls_to_scrape = []
        if homepage_html:
            ...
            for a in soup.find_all('a', href=True):
                ...
                if href.startswith('http'):
                    ...
                    # Skip other relative forms
                    continue
                # Only same-domain
                if urlparse(full).netloc == urlparse(base_url).netloc:
                    candidates.add(full)

            scored = sorted(list(candidates), key=self._score_link_priority, reverse=True)
            urls_to_scrape = scored[:12]  # candidate pool

        # Always include common paths as fallback
        common_paths = [
    ...
    def scrape_website_deep(self, base_url: str, max_pages: int = 5) -> Dict[str, any]:
        ...
        - Robots-aware with cached decisions
        - Prioritize likely service pages
        - Early stop when marginal gain is low
        """
        if self.demo_mode:
            logger.info(f"DEMO MODE: Using mock website data for {base_url}")
            data = self.get_mock_website_data(base_url)
            data.update({'blocked_by_robots': False, 'scrape_strategy': 'deep_demo'})
            return data
        ...
        if not self.can_scrape_url(base_url):
            logger.warning(f"Robots.txt disallows scraping: {base_url}")
            # Fall back to no-scrape enrichment (no requests to the domain)
            return {
                **self._enrich_without_scrape({
        ...
        try:
            ...
            response.raise_for_status()
            homepage_html = response.content
            soup = BeautifulSoup(homepage_html, 'html.parser')
            ...
            pages_to_scrape = self.discover_site_pages(base_url, homepage_html=homepage_html)
            ...
            all_text = []
            pages_scraped = 0
            no_gain_streak = 0  # early stop if consecutive pages add no services
            ...
            for page_url in pages_to_scrape:
                ...
                try:
                    ...
                    gain = len(all_services) - before
                    if gain == 0:
                        no_gain_streak += 1
                    else:
                        no_gain_streak = 0
                    if no_gain_streak >= 2:
                        # Stop early if two pages in a row add nothing
                        logger.debug("Early stopping deep scrape due to low marginal gain")
                        break
            ...
            return {
                ...
                'blocked_by_robots': False,
                'scrape_strategy': f'deep_{pages_scraped}_pages'
            }

        except Exception as e:
            logger.error(f"Error in deep scrape for {base_url}: {str(e)}")
            # Fall back to single-page scrape (which itself respects robots)
            data = self.scrape_website(base_url)
            if 'scrape_strategy' not in data:
                data['scrape_strategy'] = 'fallback_single_page'
            return data
    ...
    def calculate_ai_score(self, practice_data: Dict) -> Tuple[int, Dict[str, int], str]:
        ...
        Returns: (total_score, score_breakdown, detected_specialty)
        """
        ...
        address = practice_data.get('formatted_address', practice_data.get('address', '')).lower()
        ...
        # 2. Decision-Making Autonomy (20) - smaller is better
        ...
        corporate_indicators = ['corporate', 'chain', 'franchise', 'national', 'locations',
        ...
        service_matches = sum(1 for service in aesthetic_services if any(service in s.lower() for s in services))
        scores['aesthetic_services'] = min(service_matches * 3, 15)

        # 4. Competing Devices (10)
        competing_devices = ['coolsculpting', 'thermage', 'ultherapy', 'sculptra', 'emsculpt', 'vanquish', 'exilis']
        ...
        # 6. Reviews & Rating (10)
        rating = practice_data.get('rating', practice_data.get('review_rating', 0)) or 0
        review_count = practice_data.get('user_ratings_total', practice_data.get('review_count', 0)) or 0
        ...
        # 7. Search Visibility (10)
        website = practice_data.get('website', '')
        if website and 'http' in website:
            scores['search_visibility'] = 10
        elif website:
            scores['search_visibility'] = 7
        elif practice_data.get('formatted_phone_number') or practice_data.get('phone'):
            scores['search_visibility'] = 4
        else:
            scores['search_visibility'] = 1

        # 8. Financial Indicators (10)
        affluent_indicators = ['hills', 'park', 'lake', 'estates', 'plaza', 'center', 'avenue', 'boulevard', 'suite']
        is_affluent_area = any(indicator in address for indicator in affluent_indicators)
        cashpay_keywords = ['aesthetic', 'cosmetic', 'elective', 'spa', 'beauty', 'anti-aging', 'wellness', 'rejuvenation']
        offers_cashpay = any(keyword in all_text for keyword in cashpay_keywords)
        financial_score = (5 if is_affluent_area else 0) + (5 if offers_cashpay else 0)
        scores['financial_indicators'] = financial_score

        # 9. Weight Loss Services (5)
        weight_keywords = ['weight loss', 'medical weight', 'hormone therapy', 'iv therapy', 'body contouring', 'fat reduction', 'inch loss']
        ...
        # 10. Specialty-Specific Keyword Bonus (up to 10)
    ...
    def recommend_device(self, practice_data: Dict, ai_scores: Dict) -> Dict:
        ...
        for device_name, device_info in self.device_catalog.items():
            ...
            for specialty in device_info['specialties']:
                if any(specialty.lower() in s.lower() for s in services):
                    score += 20
            ...
            device_scores[device_name] = {'score': score, 'reasons': reasons}
    ...
    def craft_outreach_opener(self, practice_data: Dict, ai_score: int, device_rec: Dict) -> str:
        ...
        return f"Hi! I noticed {practice_name}'s excellent reputation. Many practices like yours are seeing significant results with {primary_device}. Would you be interested in a brief conversation?"

    def _compute_data_confidence(self, ai_score: int, completeness: float, blocked_by_robots: bool) -> Tuple[str, float]:
        """
        Return (confidence_label, data_confidence_score_0_1)
        """
        score_component = ai_score / 100.0
        block_penalty = 0.25 if blocked_by_robots else 0.0
        conf = max(0.0, min(1.0, 0.4 * completeness + 0.6 * score_component - block_penalty))
        if conf >= 0.7:
            label = "High"
        elif conf >= 0.45:
            label = "Medium"
    ...
    def process_practice(self, place_data: Dict) -> Optional[Dict]:
        ...
        # Scrape website if available (deep multi-page scraping with graceful fallback)
        ...
        # Calculate data completeness (strict fields)
        ...
        # Calculate confidence and data_confidence
        confidence_label, data_confidence = self._compute_data_confidence(
            ai_score, completeness, practice_record.get('blocked_by_robots', False)
        )
    ...
    def export_to_csv(self, results: List[Dict], filename: str):
        ...
        csv_columns = [
            'name', 'specialty', 'address', 'phone', 'website', 'rating', 'user_ratings_total',
            'ai_score', 'confidence_level', 'data_confidence', 'primary_device_rec', 'device_rationale',
            'outreach_opener', 'services', 'social_links', 'staff_count',
            'blocked_by_robots', 'scrape_strategy',
            ...
            'data_completeness',
            # Optional AI-enhanced fields if present on record
        ...
        for result in results:
            ...
            for col in csv_columns:
                if col.endswith('_score') and col not in ('ai_score', 'ai_readiness_score'):
...
====
High-Fit (70-100): {high_fit} ({(high_fit/total_prospects*100 if total_prospects else 0):.1f}%)
Medium-Fit (50-69): {medium_fit} ({(medium_fit/total_prospects*100 if total_prospects else 0):.1f}%)
Low-Fit (0-49): {low_fit} ({(low_fit/total_prospects*100 if total_prospects else 0):.1f}%)
...
{i}. {prospect.get('name', 'Unknown')}
   Score: {prospect.get('ai_score', 0)}/100 | Confidence: {prospect.get('confidence_level', 'Unknown')} ({prospect.get('data_confidence', 0)})
   Device Rec: {prospect.get('primary_device_rec', 'None')}
   Blocked by robots: {prospect.get('blocked_by_robots', False)} | Strategy: {prospect.get('scrape_strategy', 'n/a')}
   Address: {prospect.get('address', 'Unknown')} | Phone: {prospect.get('phone', 'Unknown')}
"""
