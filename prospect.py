    ...
    def __init__(self, demo_mode=False, existing_customers_csv=None):
        ...
        for path in possible_paths:
            if os.path.exists(path):
                config_path = path
                logger.info(f"âœ“ Found scoring config at: {path}")
                break
        ...
        if config_path:
            try:
                ...
                logger.info("âœ“ New scoring engine initialized successfully")
        ...
        try:
            ...
            for keyword in keywords:
                try:
                    ...
                    logger.info(f"  â€¢ {keyword}: ~{result_count} results")
...
def main():
    ...
    print(f"Google Places API Key: {'âœ“ Configured' if prospector.gmaps_key else 'âœ— Missing'}")
    sys.stdout.flush()
